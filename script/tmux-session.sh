...
if ! tmux has-session -t $session; then
    ...
    tmux rename-window -t $session:1 log1
    tmux select-window -t $session:1 "touch /var/tmp/logger.test-1.log; while true; do date >> /var/tmp/logger.test-1.log; sleep 1; done" Enter
#    tmux -t $session:1 "touch /var/tmp/logger.test-1.log; while true; do date >> /var/tmp/logger.test-1.log; sleep 1; done" Enter
#    tmux new-window -t $session:2 "    " Enter
