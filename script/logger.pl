...
my $wfiles = "$wdir/*.$wext";		# Files to monitor by mask
...
sub runAsClient{
    ...
    while (1 && !$ctrl_c) {
        ...
        my $reopen = ($reopenTime > 0) && ((time() - $utime) >= $reopenTime);
...
sub deliver {
    ...
    if ($doEcho && @destinations) {
        foreach $h (@destinations) {
            $h->send($s); # || die("Error writing to echo socket for $h->peerhost(): $@");
        }
    }
    ...
    for (my $i = 0; $i <= $#ARGV; $i++) {
        ...
        if ($ARGV[$i] =~ m/^-/) {
            ...
            #   -?                  -- Print help    
                ...
                print("       Options:\n");
                print("             -ccols          Chop lines at cols columns, 0 = no chop.\n");
                ...
                print("             -Dwdir          Monitor file(s) in 'wdir' directory.\n");                
                print("             -dmdays         Monitor for files modified in last 'mdays'.\n");
                print("             -wcols          Wrap lines at cols columns, 0 = no wrap.\n");
                print("             -T              Just send test messages to monitoring files.\n");
